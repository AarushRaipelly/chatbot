...
from django.contrib.auth.hashers import make_password
from .models import Session, Message
from .serializer import SessionSerializer, MessageSerializer, SessionListSerializer
from django.utils import timezone
...
def chat_view(request):
    ...
    try:
        ...
        if request.user.is_authenticated:
            # For authenticated users, ignore guest_session_id and is_guest
            if session_id:
                try:
                    # Only get sessions that belong to this user
                    session = Session.objects.get(
                        id=session_id,
                        user=request.user,
            ...
            # Create new session for authenticated user if not found
            if not session:
                session = Session.objects.create(
                    user=request.user,
                    title="New Chat",
        ...
        # Handle guest users
            ...
            if session_id:
                try:
                    # Only get guest sessions with matching guest_session_id
                    session = Session.objects.get(
                        id=session_id,
                        is_guest_session=True,
                        guest_session_id=guest_session_id,
            ...
            # Create new guest session if not found
            if not session:
                session = Session.objects.create(
                    user=None,  # No user for guest sessions
                    title="Guest Chat",
                    is_guest_session=True,
                    guest_session_id=guest_session_id
                )
        ...
        # Include guest_session_id for guest users
        if not request.user.is_authenticated:
            response_data["guest_session_id"] = guest_session_id
...
@permission_classes([IsAuthenticated])
def get_user_sessions(request):
    """Get all sessions for the authenticated user only"""
...
@permission_classes([AllowAny])
def get_sessions(request):
    """Get sessions based on user authentication status"""
    if request.user.is_authenticated:
        # Authenticated user - return only their sessions
        sessions = Session.objects.filter(
            user=request.user,
            is_guest_session=False
        ).order_by('-updated_at')
    else:
        # Guest user - return only their guest sessions
        guest_session_id = request.GET.get('guest_session_id')
        if not guest_session_id:
            return Response({"error": "Guest session ID required for guest users"}, status=400)
...
@permission_classes([AllowAny])
def session_messages(request, session_id):
    """Get messages for a specific session with proper user isolation"""
    try:
        if request.user.is_authenticated:
...
@permission_classes([AllowAny])
def delete_session(request, session_id):
    """Delete a session with proper user isolation"""
    try:
        if request.user.is_authenticated:
            ...
            # Guest user - can only delete their own guest sessions
            guest_session_id = request.data.get('guest_session_id') or request.GET.get('guest_session_id')
            if not guest_session_id:
                return Response({"error": "Guest session ID required"}, status=400)
...
@permission_classes([AllowAny])
def update_session_title(request, session_id):
    """Update session title with proper user isolation"""
    try:
        if request.user.is_authenticated:
            # Authenticated user - can only update their own sessions
            session = Session.objects.get(
                id=session_id,
                user=request.user,
                is_guest_session=False
            ...
            # Guest user - can only update their own guest sessions
            guest_session_id = request.data.get('guest_session_id') or request.GET.get('guest_session_id')
            if not guest_session_id:
                return Response({"error": "Guest session ID required"}, status=400)
...
@permission_classes([AllowAny])
def create_new_session(request):
    """Create a new session with proper user handling"""
    if request.user.is_authenticated:
        # Authenticated user - create user session
        session = Session.objects.create(
            user=request.user,
            title="New Chat",
            is_guest_session=False
        ...
        # Guest user - create guest session
        guest_session_id = str(uuid.uuid4())
        session = Session.objects.create(
            user=None,
            title="Guest Chat",
